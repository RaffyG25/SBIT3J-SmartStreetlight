...
<head>
    ...
    <title>Dashboard with Add User</title>
...
<body class="bg-gray-100 h-screen flex">
    ...
    <main class="flex-1 p-6">
        <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
            <h2 class="text-2xl font-semibold mb-6 text-gray-900 text-center">Add User</h2>
    ...
    <script>
        ...
        collapseBtn.addEventListener('click', () => {
            ...
            if (isCollapsed) {
                collapseIcon.setAttribute('d', 'M9 18l6-6-6-6'); // Change to chevron-right
            } else {
                collapseIcon.setAttribute('d', 'M15 18l-6-6 6-6'); // Change to chevron-left
            }
        });

        // Mobile menu toggle (optional - for very small screens if needed)
        ...
        function fetchRoles() {
            // Replace this with your actual server-side fetch logic (e.g., using fetch or XMLHttpRequest)
            fetch('get_roles.php')  //  <-  Replace 'get_roles.php' with your server endpoint
            ...
            .then(roles => {
                ...
                roles.forEach(role => {
                    ...
                    option.value = role.id_role;  // Store the id_role as the value
                    option.textContent = role.value; // Display the role name
                    roleSelect.appendChild(option);
                });
                // Select the first role by default.
                if (roles.length > 0) {
                    roleSelect.value = roles[0].id_role;
                }
            })
            .catch(error => {
                errorMessage.textContent = 'Failed to fetch roles: ' + error.message;
                errorMessage.style.display = 'block';
                roleSelect.innerHTML = '<option value="">Error</option>'; // keep the select element populated
            });
        }

        // Call fetchRoles when the page loads
        fetchRoles();
        ...
        form.addEventListener('submit', (event) => {
            ...
            if (isUsernameValid && isPasswordValid && isRoleValid) {
                ...
                const newUser = {
                    ...
                    id_role: roleSelect.value, // Send the selected id_role
                };

                fetch('add_user.php', {  //  <-  Replace '/api/users' with your server endpoint
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        errorMessage.textContent = data.message || 'Failed to add user.';
                        errorMessage.style.display = 'block';
                        successMessage.style.display = 'none';
                    }
                })
